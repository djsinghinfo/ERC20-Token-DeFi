...
/**
* @notice implementation of the Pixel token contract
* @author Pixel
*/
contract PixelToken is ERC20, VersionedInitializable {
    ...
    string internal constant NAME = "Pixel Token";
    string internal constant SYMBOL = "PXLD";
    uint8 internal constant DECIMALS = 18;

    /// @dev the amount being distributed for the LEND -> PXLD migration
    ...
    /// @dev reference to the PXLD governance contract to call (if initialized) on _beforeTokenTransfer
    /// !!! IMPORTANT The PXLD governance is considered a trustable contract, being its responsibility
    /// to control all potential reentrancies by calling back the PixelToken
    ITransferHook public _pxldGovernance;
    ...
    * @param migrator the address of the LEND -> PXLD migration contract
    * @param distributor the address of the PXLD distribution contract
    */
    function initialize(
        address migrator,
        address distributor,
        ITransferHook pxldGovernance
        ...
        _pxldGovernance = pxldGovernance;
    ...
    */
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal override {
        ...
        // caching the pxld governance address to avoid multiple state loads
        ITransferHook pxldGovernance = _pxldGovernance;
        if (pxldGovernance != ITransferHook(0)) {
            pxldGovernance.onTransfer(from, to, amount);
        }
    }
}